...
**Nightly retention:** listings not seen for 30 days (`RETENTION_DAYS`) are pruned along with their deals in one transaction, and the WAL is checkpointed. The window must stay comfortably above the comp engine's 14-day lookback. Set `RETENTION_DAYS=0` to keep everything forever; prunes run at most once per UTC day (deduped across restarts) and never block polling.
